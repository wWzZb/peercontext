...
Before asking, check the current repository and conversation. Run `peerctx --json agent list`, then `peerctx --json agent get AGENT` for the best plausible public Manifest. If no Agent clearly fits, ask the user instead of broadcasting.
...
Pipe the exact request bytes to `peerctx --json ask AGENT`. Treat only `data.response.answer` from a successful envelope as the remote answer. Do not blindly retry timeout, offline, authorization, host-unavailable, or mDNS errors.
